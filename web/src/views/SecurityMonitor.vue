  ...
  <div class="py-12 px-4">
    <div class="max-w-4xl mx-auto">
      ...
      <div class="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
        ...
        <div class="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
          <div class="flex items-center gap-3">
            ...
            <span class="text-[10px] font-mono text-slate-400 bg-slate-50 px-2 py-0.5 rounded">{{ alerts.length }} events</span>
          </div>
          <div class="flex items-center gap-4">
            <span class="text-xs text-slate-400 font-mono">newest first</span>
            <button @click="clearHistory" class="text-xs px-3 py-1.5 bg-red-50 text-red-600 hover:bg-red-100 hover:text-red-700 rounded-lg transition-colors font-medium">Clear History</button>
          </div>
        </div>

        <!-- Empty state -->
        <div v-if="alerts.length === 0" class="py-24 text-center">
          <div class="w-14 h-14 rounded-2xl bg-slate-50 border border-slate-200 flex items-center justify-center mx-auto mb-5">
          ...
          <p class="text-sm text-slate-500 font-medium">Waiting for detection events…</p>
          <p class="text-xs text-slate-400 mt-2 font-mono">Use the Simulator tab or run: node simulate-attack.js</p>
        </div>

        <!-- Log rows -->
        <div v-else class="divide-y divide-slate-100 max-h-[70vh] overflow-y-auto" style="scrollbar-width: thin; scrollbar-color: #cbd5e1 transparent;">
          <div
            v-for="(alert, i) in alerts"
            :key="alert.timestamp + '-' + i"
            class="px-6 py-5 flex items-center gap-5 transition-colors duration-150"
          ...
          >
            ...
            <div
              ...
              :class="alert.prediction === 1 ? 'bg-red-100 border border-red-200' : 'bg-emerald-50 border border-emerald-200'"
            >
              <svg v-if="alert.prediction === 1" xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 text-red-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
                <path stroke-linecap="round" stroke-linejoin="round" d="M12 9v2m0 4h.01M10.29 3.86L1.82 18a2 2 0 001.71 3h16.94a2 2 0 001.71-3L13.71 3.86a2 2 0 00-3.42 0z" />
              </svg>
              <svg v-else xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 text-emerald-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
            ...
            <div class="flex-1 min-w-0">
              <div class="flex justify-between text-xs text-slate-400 mb-2">
                <span class="font-medium text-slate-500">Confidence</span>
                <span
                  class="font-bold"
            ...
            <!-- Metrics -->
            <div class="flex flex-col gap-1.5 lg:gap-5 text-xs font-mono flex-shrink-0">
              <div class="flex gap-3 sm:gap-4 md:gap-5">
                <span class="text-slate-400">tx=<span class="text-slate-700 font-semibold">{{ alert.txCount }}</span></span>
                <span class="text-slate-400">senders=<span class="text-slate-700 font-semibold">{{ alert.uniqueSenders }}</span></span>
                <span class="text-slate-400">
                  pair_ratio=<span :class="alert.samePairRatio > 0.5 ? 'text-red-500 font-bold' : 'text-slate-700 font-semibold'">
      ...
      <p class="text-center text-xs text-slate-400 mt-6">
        Classification runs every 60 seconds · powered by Linear SVM
      </p>
